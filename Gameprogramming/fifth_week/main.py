...
class Game:
    ...
    def update(self):
        ...
        while len(self.platforms) < 6:
            ...
            width = random.choice(self.width_size)#50, 80 100 size
            p = Platform(random.randrange(0, WIDTH-random_width),
                         random.randrange(35, 60),
                         width, 20)
            self.platforms.add(p)
            self.all_sprites.add(p)
    ...
    def load_date(self):
        ...
        self.move = Spritesheet(os.path.join(img_dir, MOVE))
        self.bullet = Spritesheet(os.path.join(img_dir, BULLET))
        self.enemy = Spritesheet(os.path.join(img_dir, ENEMY))
        self.jump = Spritesheet(os.path.join(img_dir, JUMP))

    def draw(self):
        #game loop - draw
        self.screen.blit(gameImg, (0, 0))
